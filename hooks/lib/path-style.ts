...
/** Test whether a name matches a flat date markdown file (e.g. "2026-04-03.md"). */
export function isFlatDateFile(name: string): boolean {
  return FLAT_DATE_FILE_PATTERN.test(name);
}

/** Test whether a name matches a compact mm-dd markdown file (e.g. "04-03.md"). */
export function isCompactDateFile(name: string): boolean {
  return COMPACT_DATE_FILE_PATTERN.test(name);
}

/** Test whether a name matches a mm-Name or dd-Name markdown file (e.g. "04-April.md"). */
export function isNumNameFile(name: string): boolean {
  return NUM_NAME_FILE_PATTERN.test(name);
}

/** Test whether a name matches a bare two-digit day markdown file (e.g. "03.md"). */
